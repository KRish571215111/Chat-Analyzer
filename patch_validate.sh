#!/bin/bash
cat << 'INNER_EOF' >> app/src/main/java/com/example/utils/ReportGenerator.kt
            ...
            imgRegex.findAll(content).forEach { match ->
                ...
                if (!src.startsWith("http") && !src.startsWith("data:")) {
                    ...
                    if (!imgFile.exists()) {
                        ...
                        // Fix: change broken link to a placeholder or data URI
...
INNER_EOF
# Remove the existing trailing brace of the object (assuming it's at the end of the file)
# Since I used cat >>, I appended to the end of the file, which means it will be outside the object.
# I need to insert it INSIDE the object.
